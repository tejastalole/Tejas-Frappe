frappe.ui.form.on("Biometric Device", {
	refresh(frm) {
		...
		const mode = frm.doc.connection_mode || "ADMS Push";
		const tcp_allowed = mode === "TCP Pull" || mode === "Both";
		...
		if (tcp_allowed) {
			...
			frm.dashboard.set_headline_alert(
				__(
					"ADMS Push mode: device must send punches to /iclock on this server. TCP Pull buttons are hidden."
				),
				"blue"
			);
		}
